...
下图只使用 Uni debug report 中标记为 `result="ok"` 的完整记录，不与 Make 混合比较。总耗时由报告中的 `elapsed` 读取；阶段柱状图由 `graph-analysis`、`kernel` 和 `final` 三个阶段耗时组成。
...
同一目标、源码、缓存状态和硬件下，耗时下降率可以按两次 Uni 记录计算：

```text
下降率 = 1 - 新记录耗时 / 旧记录耗时
```

这些记录来自不同日期，构建图、缓存命中、源码变化、温度、后台负载、存储缓存、swap 和目标状态都可能不同。失败、被中断、只构建单个目标或无法确认阶段完整性的记录不进入图表，因此图表用于展示 Uni 的日志趋势，不是跨设备性能承诺。
